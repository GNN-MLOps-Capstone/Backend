...
async def run_volatility_check():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            sent_history: set[tuple] = set(sent_result.all())
    ...
    for stock_code, stock_name in all_stocks:
        ...
        try:
            ...
            current_type = "high_risk" if abs_rate >= 10.0 else "risk"
             ...
             # 이 종목의 watcher 전체 조회
            async with AsyncSessionLocal() as db:
                user_stmt = select(User.google_id, User.id).distinct().join(
            ...
            if not user_rows:
                continue
            def already_notified(user_id: int, stype: str) -> bool:
                if stype == "high_risk":
                    # high_risk 전송 대상: high_risk를 아직 안 받은 유저
                    return (user_id, stock_name, "high_risk") in sent_history
                else:  # risk
                    # risk 전송 대상: risk도 high_risk도 아직 안 받은 유저
                    return (
                        (user_id, stock_name, "risk") in sent_history
                        or (user_id, stock_name, "high_risk") in sent_history
                    )
 
            target_users = [
                google_id
                for google_id, user_id in user_rows
                if not already_notified(user_id, current_type)
            ]
 
            if not target_users:
                continue
 
            await send_volatility_push_and_save(target_users, stock_name, rate)
 
            # ✅ 발송 후 메모리 상의 sent_history 업데이트 (같은 루프 내 중복 방지)
            for google_id, user_id in user_rows:
                if google_id in target_users:
                    sent_history.add((user_id, stock_name, current_type))
